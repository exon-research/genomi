...
| Browser upload for genome source files | `Attach records` uploads clinical records and other small files into the project workspace, but genome sources are intake by local path only. `genomi.parse_source` takes a filesystem path, and the browser import route caps a body at 4 MB, so a 23andMe export, VCF, BAM, or FASTQ cannot be handed to Genomi the same way a records PDF can. The `Add your genome` starter card therefore has to ask for a path while the composer beside it does a real upload. Do not describe genome intake as an upload until this exists. | Chunked/streamed local genome-source intake with a resumable upload job, digest verification, and a browser-visible parse readiness state that shares the `Attach records` mental model. |
| Pinned composer over a scrolling conversation region | The research pane is one scrolling document in the order intake, composer, attached records, transcript, review, investigation board. The composer therefore sits above the transcript, so the newest answer in a long longitudinal investigation lands at the bottom of the scroll while the composer scrolls out of view, and `scrollBottom()` in `portal_messages.js` is inert because `#messages` is not the scroll container. Correcting this is a pane layout change, not a copy change: it needs a scroll region wrapping intake, attachments, transcript, review, and the investigation board, a composer pinned beneath it, and reworked direct-child CSS plus mobile breakpoints. | Conversation scroll region with a pinned composer, a working newest-turn scroll anchor, and the investigation board kept inside the scrolled reading flow. |
| Conversation binding for records attached before the first turn | Imported project files carry an origin conversation, and `Attach records` sends the open conversation with the import so the transcript can show `Records in this conversation`. A record attached before the first turn exists has no conversation yet; it is visible on the sent user message as selected material, but it is not bound to the conversation the turn creates. | A deferred attachment binding that adopts pre-turn imports into the conversation the first turn creates, through the same server-owned run-create path. |
