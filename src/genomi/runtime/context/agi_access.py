...
def agi_access_status(
    ...
    *,
    ...
    # Registry presence is not approval. Keep accepting the shared runtime
    # context argument without allowing a selected/default user to bypass the
    # explicit session grant recorded in context.
    _ = registry
    state = context if context is not None else load_context(root)
