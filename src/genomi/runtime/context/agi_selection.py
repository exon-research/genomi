...
from .agi_registry import _find_agi, _register_agi_revision
...
def set_active_agi_from_source(
    ...
    *,
    ...
    with context_authority_lock(root):
        ...
        previous = context.get("agis", {}).get(agi_id) or registry.get(
            "agis", {}
        ).get(agi_id)
        ...
        context["shared_evidence_db"] = run.get("shared_evidence_db") or _path_str(
            shared_evidence_db_path(root)
        )
        if user_nickname:
